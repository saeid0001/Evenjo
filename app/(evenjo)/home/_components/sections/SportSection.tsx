...
import { getSportSortBycategory } from "@/app/lib/server";
import { Sport } from "@/app/lib/types/event";
...
const SportSection = <T extends Sport>({ data, title }: Props<T>) => {
  ...
  const getSearchParams = searchParams.get("sport");
  ...
  const { data: dataConcert, isLoading } = useQuery({
    queryKey: ["sport", getSearchParams],
    queryFn: () => getSportSortBycategory(getSearchParams || "All"),
  });
        ...
        {getAllCategory.map((cat) => {
          return (
            <button
              ...
              className={`
                ${
                  searchParams.get("sport") === null && cat === "All"
                    ? " text-tint-400 bg-gradient-cool border-tint-400"
                    : ""
                }
                ${
                  searchParams.get("sport") === cat
                    ? " text-tint-400 bg-gradient-cool border-tint-400"
                    : ""
