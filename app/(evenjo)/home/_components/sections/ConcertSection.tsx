...
import { getConcertsSortBycategory } from "@/app/lib/server";
import { Concert } from "@/app/lib/types/event";
...
const ConcertSection = <T extends Concert>({ data, title }: Props<T>) => {
  ...
  const { data: dataConcert, isLoading } = useQuery({
    queryKey: ["concert", getSearchParams],
    queryFn: () => getConcertsSortBycategory(getSearchParams || "All"),
  });
            ...
            return (
              ...
              >
                <div className="relative w-full h-84 rounded-four overflow-hidden">
                  ...
                  <Image
                    src={value.imagesinger[0]}
                    alt={value.namesinger}
                    fill
